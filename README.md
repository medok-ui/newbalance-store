...
<p>
  <img src="https://img.shields.io/badge/status-%20работает-green?style=flat-square" />
  <img src="https://img.shields.io/badge/адаптив-%20есть-red?style=flat-square" />
...
Пет-проект интернет-магазина в стиле **New Balance**: каталог кроссовок, фильтры, корзина, избранное, оформление заказа с реальной интеграцией оплаты (в тестовом режиме) и полноценная админ-панель для управления товарами, заказами и пользователями.

Сделано на **Angular 21** (standalone + signals + SSR) с бэкендом на **Supabase**.

---

## <img src="https://api.iconify.design/lucide/sparkle.svg?color=%23D4FF00" width="20" valign="middle"/> Что нового

С момента прошлого обновления описания добавились:

- <img src="https://api.iconify.design/lucide/key-round.svg?color=%23D4FF00" width="14"/> **Восстановление пароля по email** — запрос письма со ссылкой, страница задания нового пароля, ограничение на повторную отправку (таймер 60 сек)
- <img src="https://api.iconify.design/lucide/mail.svg?color=%23D4FF00" width="14"/> **Смена email** в настройках профиля с подтверждением на новый адрес
- <img src="https://api.iconify.design/lucide/frown.svg?color=%23D4FF00" width="14"/> **Страница 404** для несуществующих маршрутов
...
<tr><td><img src="https://api.iconify.design/lucide/key-round.svg?color=%23D4FF00" width="18"/></td><td>Восстановление и смена пароля по ссылке из письма</td></tr>
<tr><td><img src="https://api.iconify.design/lucide/mail.svg?color=%23D4FF00" width="18"/></td><td>Смена email с подтверждением на новый адрес</td></tr>
...
<tr><td><img src="https://api.iconify.design/lucide/user.svg?color=%23D4FF00" width="18"/></td><td>Личный кабинет: профиль, адрес доставки, смена пароля/email, история заказов</td></tr>
...
<tr><td><img src="https://api.iconify.design/lucide/frown.svg?color=%23D4FF00" width="18"/></td><td>Страница 404 для несуществующих маршрутов</td></tr>
</table>

### <img src="https://api.iconify.design/lucide/shield.svg?color=%23D4FF00" width="18" valign="middle"/> Админ-панель подробнее

Доступна по отдельному входу (`/dashboard/login`), защищена собственным guard'ом (проверка по таблице `admins`).

- **Дашборд** — карточки со статистикой (кол-во товаров, заказов, пользователей), таблица последних заказов, топ-10 продаваемых товаров
- **Товары** — список с поиском, создание нового товара (основная информация, бренд/категория/пол/метка, характеристики, описание, размеры, загрузка фото drag & drop, автоподсчёт скидки), редактирование существующего товара, удаление
- **Заказы** — список всех заказов, детальная страница заказа: данные покупателя, состав заказа, попытки оплаты (Stripe session id), смена статуса (в работе / выполнен / отменён), удаление заказа
- **Пользователи** — список всех зарегистрированных пользователей с их профилями (получается через Edge Function `get-users`, объединяющую `auth.users` и `profiles`), поиск по имени
...
> Для полноценной работы нужен настроенный проект **Supabase** (Auth, таблицы, Storage, Edge Functions `create-payment` и `get-users`) — см. `src/environments/environment.ts` и `supabase/config.toml`.
...
| | |
|---|---|
| <img src="https://api.iconify.design/lucide/check.svg?color=%23D4FF00" width="16"/> | Основной функционал магазина и админ-панели работает стабильно |
| <img src="https://api.iconify.design/lucide/check.svg?color=%23D4FF00" width="16"/> | Адаптивная вёрстка есть в пользовательской части магазина |
| <img src="https://api.iconify.design/lucide/monitor.svg?color=%23D4FF00" width="16"/> | Адаптива в админ-панели не будет — это рабочий инструмент для ПК/ноутбуков, не рассчитанный на мобильные устройства |
| <img src="https://api.iconify.design/lucide/check.svg?color=%23D4FF00" width="16"/> | Есть страница 404 для несуществующих маршрутов |
| <img src="https://api.iconify.design/lucide/globe.svg?color=%23D4FF00" width="16"/> | Интерфейс только на русском языке |
| <img src="https://api.iconify.design/lucide/hourglass.svg?color=%23D4FF00" width="16"/> | Раздел «Категории» в админке пока заглушка |
